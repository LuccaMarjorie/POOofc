...
class NotaTexto extends Nota {
    ...
    @Override
    public void salvar() {
        // Implementação será feita na classe de persistência
    }
...
// Classe para itens do checklist
class ItemChecklist implements Serializable {
    ...
    public boolean isConclusao() { return conclusao; }
    public void setConclusao(boolean conclusao) { this.conclusao = conclusao; }
}

// Classe para checklists
...
// Classe para notas de data comemorativa (MODIFICADA)
class DataComemorativa extends Nota {
    private String ocasiao;
    private Date dataEvento; // NOVO CAMPO: data específica do evento
    ...
    // NOVOS GETTERS E SETTERS PARA DATA DO EVENTO
...
class Usuario implements Serializable {
    ...
    public void criarNota(Nota nota) {
        notas.add(nota);
        salvarDados(); // Salva automaticamente ao criar nova nota
    }
    ...
    public void excluirNota(int indice) {
        if (indice >= 0 && indice < notas.size()) {
            notas.remove(indice);
            salvarDados(); // Salva automaticamente ao excluir nota
        }
    }
